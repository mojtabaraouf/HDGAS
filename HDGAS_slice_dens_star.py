...
for snap in range(1,101):
    ...
    for i, (direction, field) in enumerate(zip(cuts, fields)):
        ...
        p.annotate_particles((200, 'pc'),p_size=1,marker='1',ptype='PartType4',col='b', alpha=0.01,stride=1)
        p.annotate_particles((10, 'pc'),p_size=50,marker='o',ptype='PartType5')

        p.set_cmap(field=("gas", "Density"), cmap=cmap)
        # p.annotate_scale(corner='upper_right')
        p.set_zlim(("gas", "Density"),1e-2, 1e6)
        ...
        p._setup_plots()
        jj +=1
    savename = Dens+"Map_Density_z_%03d.png" % snap
    p.save(savename)
